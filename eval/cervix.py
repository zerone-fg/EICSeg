...
def prepare_model_painter(chkpt_dir, arch='painter_vit_large_patch16_input896x448_win_dec64_8glb_sl1', args=None):
    import xdecoder.models_painter as models_painter
...
def run_one_image_painter(img, tgt, size, model, out_path, device):
    x = torch.tensor(img)
    # x = x.unsqueeze(dim=0)
    x = torch.einsum('nhwc->nchw', x)
...
def inference_multi_painter(model, image, label_onehot, support_images, support_labels_onehot, device):
    import json
    color_dict = define_colors_by_mean_sep(num_colors=133, channelsep=7)
    n_labels, h, w = label_onehot.shape  ### (5, 448, 448)
    image, label_onehot = image.to(device), label_onehot.to(device)
    ...
    output = run_one_image_painter(img, tgt, [448, 448], model, "painter.png", device)
    return output
