...
@torch.no_grad()
def inference_multi_our(model, image, label_onehot, support_images, support_labels_onehot, device, alpha):
    ...
    # save_img.save("pan/test_image_{}.png".format(1 * 5 + i))
    ...
    for id in range(n_labels):
        save_mask[hard_pred[id][0] == 1] = torch.tensor(id + 1, dtype=torch.uint8)
        mask = hard_pred[id][0]
        mask[hard_pred[id][0] == 1] = torch.tensor(id + 1, dtype=torch.uint8)
...
def post_preprocess(mask, train_img):
    ...
    from xdecoder.body.ScribblePrompt.segment_anything.build_sam import sam_model_registry
    from xdecoder.body.ScribblePrompt.segment_anything import SamPredictor
    from xdecoder.body.ScribblePrompt.scribbleprompt import ScribblePromptSAM
    import cv2
...
def inference_multi_our_1(model, image, label_onehot, support_images, support_labels_onehot, device):
    label_onehot = label_onehot[1:]
    support_labels_onehot = support_labels_onehot[:, 1:]
    ...
    # for id in range(n_labels):
    #     save_mask[hard_pred_1[id][0]] = torch.tensor(id + 2, dtype=torch.uint8)
    #     mask = hard_pred_1[id][0]
    #     mask[hard_pred_1[id][0]] = torch.tensor(id + 2, dtype=torch.uint8)
    #     mask = mask.cpu().numpy().astype(np.uint8)
    #     save_colored_mask(mask, os.path.join('{}_{}_.png'.format(id, id)))
    ...
    # post_preprocess(soft_pred_onehot[1].unsqueeze(0), train_img)
    for k in range(n_labels):
...
def inference_multi(model, image, label_onehot, support_images, support_labels_onehot, device):
    ...
    #     save_mask[hard_pred_1[id][0] == 1] = torch.tensor(id + 48, dtype=torch.uint8)
    #     mask = hard_pred_1[id][0]
    #     mask[hard_pred_1[id][0] == 1] = torch.tensor(id + 13, dtype=torch.uint8)
    #     mask = mask.cpu().numpy().astype(np.uint8)
    #     # save_colored_mask(mask, os.path.join('uni{}_{}_.png'.format(id, id)))

    # save_colored_mask(np.array(save_mask), os.path.join('/newdata3/xsa/ICUSeg/mambamodel/eval/pandental_com/final_uni{}_{}_{}.png'.format(id, str(name).split("/")[-1], np.mean(scores))))
    
    # backmask = label_onehot[1:].sum(dim=0) == 0
    # label_onehot_save = torch.argmax(label_onehot, dim=0) + 48
    # label_onehot_save[backmask] = 0
    # save_colored_mask(np.array(label_onehot_save.cpu()), os.path.join('/newdata3/xsa/ICUSeg/mambamodel/eval/pandental_com/label_uni{}_{}_{}.png'.format(id, str(name).split("/")[-1], np.mean(scores))))
...
check_decoder = torch.load("/newdata3/xsa/ICUSeg/mambamodel/output_dir_dino_spsize448_onlydino/16600/checkpoint-16600_0.6383728924603416.pth")
...
for rep in range(repetation):
    ...
    for i in tqdm(idxs):
        ...
        # vals_ours_2 = inference_multi_our(model_our_1, image, label, support_images, support_labels, 'cuda', alpha=1)
